...
@frappe.whitelist()
def update_rediology_payment_status(custom_payment_id):
    ...
    for encounter in encounters:
        ...
        for lab_test in patient_encounter.custom_radiology_items:
            # Check if the lab test is already Fully Paid
            if lab_test.radiology_status != "Fully Paid":
                ...
                if all(invoice.outstanding_amount <= 0 for invoice in matching_invoices):
                    lab_test.radiology_status = "Fully Paid"
                    lab_test.results_status = "Processing Results"
                    lab_test.invoiced = 1
                    lab_test.lab_test_created = 1
                    updated = True
                    investigations.append(lab_test)
                    
                    # Create a new Lab Test document
                    create_radiology_test(lab_test, patient_encounter.patient,
...
def create_radiology_test(lab_test, patient,patient_encounter,date,practitioner,medical_department):
    # Fetch the Patient document to get gender 
    patient_doc = frappe.get_doc('Patient', patient)
    procedure_doc = frappe.new_doc('Observation')
    procedure_doc.observation_template = lab_test.radiology_investigation
    ...
    procedure_doc.custom_patient_encounter_id = patient_encounter
    procedure_doc.save()
    frappe.db.commit()  # Commit changes to the database
